...
- Mobile-shell-specific UI tests (screenshot/app-qa coverage of `MobileShell`, `RemoteConnectPanel`) - `scripts/screenshot/specs/mobile-shell-review.spec.tsx` now also captures `RemoteConnectPanel`'s idle "Connect to managed instance" state (it renders unconditionally below `MobileShell`'s local review content, so no SSH/control-plane mocking is needed for this step). Coverage of the connected/error states still requires mocking the control-plane calls (`ensureMobileDeviceKey`/`registerClientKey`/`getInstanceStatus`/`issueCertificate`) and remains open.
...
Not yet done: verification on real Android/iOS devices or simulators (not available in this environment - same caveat desktop's Phase 8 terminal carries for its own review-only verification), and a screenshot/app-qa spec for this screen specifically (it needs a connected `SshEndpoint` and running remote sessions, which the current screenshot harness's local-jj-repo fixtures don't provide - would need control-plane/SSH mocking similar to the Phase 6 gap noted above).
