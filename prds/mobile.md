...
### Phase 8: Full PTY streaming and reattach - desktop client (partial: backend commands done and tested, no UI)
...
- **Not done:** any UI. Desktop's remote-review surface has no terminal screen/panel at all yet (Phase 8's originally-scoped "surface reattach in the desktop remote-review UI: on reconnecting or reopening, list running sessions and offer reattach" needs that UI to exist first, and building a full desktop terminal panel - a local-PTY-style renderer wired to `remote_pty_create`/`remote_pty_reattach`/the `remote-pty-*` events - was out of scope for this pass given the time available). The backend commands above are ready for that UI to call once it exists.
