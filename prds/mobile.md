...
- The `ios-build` CI job (`pod install` + `xcodebuild build` against the real Xcode project) has not actually been run anywhere yet - this development sandbox has no macOS/Xcode, so only locally-checkable proxies (pbxproj brace-balance, Info.plist XML validity) were verified before committing. `android-build`'s equivalent (`compileDebugKotlin`) *was* run locally and passed.
- No full linked APK/IPA: Android compilation stops at Kotlin/Java (confirmed via a `configureCMakeDebug[arm64-v8a]` failure on a full local `assembleDebug` attempt - expected, since this sandbox has no Android NDK to build React Native's own native C++ libraries). Building real per-ABI `libtreq_mobile_ssh.so` files for a device (`cargo ndk`) is also not done.
- No UI for the "biometrics not set up" / secure-storage-unavailable case.
- The control-plane client (`controlPlane.ts`, `authStore.ts`, `certRenewal.ts`) is unit-tested against a mocked Supabase client only - not exercised against a real local Supabase stack. This is an environment limitation, not a design gap: the local Supabase CLI stack needs a reachable Docker daemon, and this sandbox's Docker socket isn't reachable (`docker info` fails) - see `scripts/service-qa/up.sh` for what that stack looks like when it can be brought up.
