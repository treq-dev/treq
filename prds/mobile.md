...
Closed since the last pass: a full linked Android APK. Installed the Android NDK (`ndk;26.1.10909125`), `cargo-ndk`, and the Android Rust targets, and added `android/app/build.gradle`'s `buildTreqMobileSshNativeLibs` task (`cargo ndk build --release` across all four ABIs, wired into `preBuild`) so `assembleDebug` links the real `libtreq_mobile_ssh.so` into `jniLibs` alongside React Native's own native libraries - verified by unzipping the resulting `app-debug.apk` and confirming `lib/<abi>/libtreq_mobile_ssh.so` for all four ABIs. Getting there needed two more fixes, both committed: disabling React Native's New Architecture (`newArchEnabled=false` - this app's bridge module needs neither TurboModules nor Fabric) and pinning AGP to 8.7.2 in `android/build.gradle` (the previously-unversioned dependency resolved to 8.6.0 via the React Native Gradle plugin, which has a real bug failing `configureCMakeDebug` for any autolinked native module - `react-native-screens` here - with `[CXX1210] No compatible library found`, even though the underlying `prefab` CLI call it logs succeeds when run by hand).
