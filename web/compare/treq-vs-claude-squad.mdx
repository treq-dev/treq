...
Treq is a desktop AI workspace manager with PR-style review, stacked workspaces, and automatic rebasing. Claude Squad is a terminal-native manager that uses tmux sessions and git worktrees for people who want minimal abstraction.

## At a Glance

| | Treq | Claude Squad |
| --- | --- | --- |
| Positioning | AI workspace manager for busy developers | Terminal-native manager for multiple AI terminal agents |
...
**Treq** gives each agent workspace its own working copy, terminal, and review surface. You inspect diffs, leave comments, and send feedback back to the agent. Dependent workspaces can form a stack. Rebases and conflicts are product features, not a manual side process.
